...
import com.grad.htmlunit.core.HttpMethod;
...
import com.grad.htmlunit.core.MockWebConnection;
import com.grad.htmlunit.core.MockWebConnection.RawResponseData;
import com.grad.htmlunit.core.WebClient;
import com.grad.htmlunit.core.WebRequest;
 ...
 * <p>
 * By default, this test runs with HtmlUnit, but this behavior can be changed by
 * having a property file named "test.properties" in the HtmlUnit root
 * directory. Sample:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * The file should contain four properties: "browsers", "ie.bin", "ff38.bin",
 * and "chrome.bin".
 * <ul>
 * <li>browsers: is a comma separated list contains any combination of "hu" (for
 * HtmlUnit with all browser versions), "hu-ie8", "hu-ff31", "ff31", "ff38",
 * "ie8", "ie11", "chrome", which will be used to driver real browsers, note
 * that you can't define more than one IE as there is no standard way to have
 * multiple IEs on the same machine</li>
 * <li>ie.bin (mandatory if it does not exist in the <i>path</i>): is the
 * location of the IEDriverServer binary (see
 * <a href="http://selenium-release.storage.googleapis.com/index.html">
 * IEDriverServer downloads</a>)</li>
 * <li>ff38.bin (optional): is the location of the FF binary, in Windows use
 * double back-slashes</li>
 * <li>chrome.bin (mandatory if it does not exist in the <i>path</i>): is the
 * location of the ChromeDriver binary (see
 * <a href="http://chromedriver.storage.googleapis.com/index.html">Chrome Driver
 * downloads</a>)</li>
 * <li>autofix (optional): if {@code true}, try to automatically fix the real
 * browser expectations, or add/remove {@code @NotYetImplemented} annotations,
 * use with caution!</li>
 * </ul>
 * </p>
 *
 * @version $Revision: 10872 $
...
public abstract class WebDriverTestCase extends WebTestCase {
	...
	/**
	 * Override this function in a test class to ask for STATIC_SERVER2_ to be
	 * set up.
	 * 
	 ...
	 * 
	 * @param useRealBrowser
	 *            whether to use real browser or not
	 */
	...
	protected void startWebServer(final MockWebConnection mockConnection) throws Exception {
		...
		if (STATIC_SERVER2_ == null && needThreeConnections()) {
			...
			/*
			 * The mock connection servlet call sit under both servers, so long
			 * as tests keep the URLs distinct.
			 */
		}
	}

	/**
	 * Returns whether to use basic authentication for all resources or not. The
	...
	/**
	 * Starts the web server on the default {@link #PORT}. The given
	 * resourceBase is used to be the ROOT directory that serves the default
	 * context.
	 * <p>
	 * <b>Don't forget to stop the returned HttpServer after the test</b>
	 *
	 * @param resourceBase
	 *            the base of resources for the default context
	 * @param classpath
	 *            additional classpath entries to add (may be null)
	 * @param servlets
	 *            map of {String, Class} pairs: String is the path spec, while
	 *            class is the class
	 * @throws Exception
	 *             if the test fails
	 */
	...
	/**
	 * Starts the <b>second</b> web server on the default {@link #PORT2}. The
	 * given resourceBase is used to be the ROOT directory that serves the
	 * default context.
	 * <p>
	 * <b>Don't forget to stop the returned HttpServer after the test</b>
	 *
	 ...
	 * context.
	 * <p>
	 * <b>Don't forget to stop the returned Server after the test</b>
	 *
	 * @param resourceBase
	 *            the base of resources for the default context
	 * @param classpath
	 ...
	 *            class is the class
	 * @param handler
	 *            wrapper for handler (can be null)
	 * @throws Exception
	 *             if the test fails
	 */
	protected void startWebServer(final String resourceBase, final String[] classpath,
	...
	public static class MockWebConnectionServlet extends HttpServlet {
		...
		private void doService(final HttpServletRequest request, final HttpServletResponse response) throws Exception {
			...
			try {
				...
				// Jetty 8.1.7 throws it in getParameterNames for a query like
				// "cb=%%RANDOM_NUMBER%%"
	...
	/**
	 * Same as {@link #loadPageWithAlerts2(String)}... but doesn't verify the
	 * alerts.
	 * 
	 * @param html
	 *            the HTML to use
	 ...
	 *            the HTML to use
	 * @param url
	 *            the url to use to load the page
	 * @return the web driver
	 * @throws Exception
	 *             if something goes wrong
	 */
	 ...
	 *             if something goes wrong
	 */
	protected final WebDriver loadPage2(final String html, final URL url, final String contentType,
		...
		if (!(driver instanceof HtmlUnitDriver)) {
			try {
				...
				// ChromeDriver version 0.5 (Mar 26, 2013) does not support the
				// setSize command
	 ...
	 * 
	 * @param html
	 *            the HTML to use for the default response
	 * @param servlets
	 *            the additional servlets to configure with their mapping
	 * @return the web driver
	 * @throws Exception
	 *             if something goes wrong
	 */
	...
	/**
	 * Same as {@link #loadPage2(String, URL)}, but with additional servlet
	 * configuration.
	 * 
	 * @param html
	 *            the HTML to use for the default page
	 * @param url
	 *            the URL to use to load the page
	 * @param servlets
	 *            the additional servlets to configure with their mapping
	 ...
	 * and loads the page with this URL using the current WebDriver version;
	 * finally, asserts that the alerts equal the expected alerts (in which
	 * "§§URL§§" has been expanded to the default URL).
	 * 
	 * @param html
	 *            the HTML to use
	 * @return the web driver
	 ...
	 *            the HTML to use
	 * @param maxWaitTime
	 *            the maximum time to wait to get the alerts (in millis)
	 * @return the web driver
	 * @throws Exception
	 *             if something goes wrong
	 */
	...
	/**
	 * Same as {@link #loadPageWithAlerts2(String)}, but specifying the default
	 * URL.
	 * 
	 * @param html
	 *            the HTML to use
	...
	/**
	 * Same as {@link #loadPageWithAlerts2(String, long)}, but specifying the
	 * default URL.
	 * 
	 * @param html
	 *            the HTML to use
	 ...
	 * 
	 * @param maxWaitTime
	 *            the maximum time to wait for the expected alert to be found
	 * @param driver
	 *            the driver instance
	 * @param expectedAlerts
	 *            the expected alerts
	...
	/**
	 * Same as {@link #loadPageWithAlerts2(String)} with additional servlet
	 * configuration.
	 * 
	 * @param html
	 *            the HTML to use for the default response
	...
	/**
	 * Same as {@link #loadPageWithAlerts2(String, URL, long)}, but with
	...
	/**
	 * Loads the provided URL serving responses from
	 * {@link #getMockWebConnection()} and verifies that the captured alerts are
	 * correct.
	 * 
	 * @param url
	 *            the URL to use to load the page
	...
	/**
	 * Gets the alerts collected by the driver. Note: it currently works only if
	 * no new page has been loaded in the window
	 * 
	 * @param driver
	 *            the driver
	...
	/**
	 * Reads the number of JS threads remaining from unit tests run before. This
	 * should be always 0, if {@link #isWebClientCached()} is {@code false}.
	 */
	@Before
	public void before() {
	...
	/**
	 * Release resources but DON'T close the browser if we are running with a
	 * real browser. Note that HtmlUnitDriver is not cached by default, but that
	 * can be configured by {@link #isWebClientCached()}.
	 */
	@After
	@Override
	 ...
	 *
	 * <p>
	 * This is needed to be {@code true} for huge test class, as we could run
	 * out of sockets.
	 *
...
/**
 * As HtmlUnit didn't generate the right events, WebDriver did it for us, but
 * now that we do it correctly, WebDriver shouldn't do it anymore
