...
import com.grad.htmlunit.util.MimeType;
...
class XmlSerializer {
    ...
    protected void printOpeningTag(final DomElement node) throws IOException {
        ...
        for (final Map.Entry<String, DomAttr> entry : attributes.entrySet()) {
            ...
            buffer_.append(com.grad.htmlunit.util.StringUtils.escapeXmlAttributeValue(value));
            buffer_.append('"');
        }
    }
    ...
    private File createFile(final String url, final String extension) throws IOException {
        ...
        name = com.grad.htmlunit.util.StringUtils.sanitizeForFileName(name);
